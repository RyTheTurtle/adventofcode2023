...
pub fn part_1(input: &Vec<String>) -> u64 {
    ...
    for line in input {
        ...
        let counts: Vec<usize> = parts[1].clone().to_string().split(",").map(|c| c.parse::<usize>().unwrap()).collect();
...
fn get_arrangements(input: String, nums: Vec<usize>) -> Result<u64, regex::Error> { 
    ...
    match broken_re.find(&input) { 
        Some(m) => {
            ...
            result += arrangements;
            result += get_arrangements(input[m.range().start+expected_springs..].to_string(), nums[1..].to_vec()).unwrap();
        }, 
        None => {
            return Ok(result);
        }
...
fn validate_arrangements(input: String, num: usize) -> u64 { 
    ...
    match input.find("?"){ 
        ...
        Some(i) => { 
            return validate_arrangements(input.replacen("?", "#", 1), num) + validate_arrangements(input.replacen("?", ".", 1),num);
        }
    }
}